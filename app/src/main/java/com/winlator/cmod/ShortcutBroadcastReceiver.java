    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action != null && action.equals("com.winlator.SHORTCUT_ADDED")) {
            ...
            if (isShortcutAdded) {
                ...
                Toast.makeText(context, "抱歉，您的设备可能不受支持", Toast.LENGTH_SHORT).show(); // yeah. I'm at a loss here.
            } else {
        ...
        if (shortcutName != null && shortcutIcon != null && shortcutIntent != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                if (shortcutManager != null && shortcutManager.isRequestPinShortcutSupported()) {
                    ...
                    if (result) {
                        Toast.makeText(context, "已通过 BroadcastReceiver 成功添加快捷方式！", Toast.LENGTH_SHORT).show();
                    } else {
                ...
                try {
                    ...
                    Toast.makeText(context, "已成功添加快捷方式（广播）。", Toast.LENGTH_SHORT).show();
                } catch (Exception e) {
